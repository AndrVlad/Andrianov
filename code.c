...
static std::queue<int> *QueueThreads;
static int interval;
static int _ticked;
...
void switch_to_next_thread()
{
    ...
    if (QueueThreads->size())
    {
        _current_thread_id = QueueThreads->front();
        QueueThreads->pop();
    }
    else
    {
...
{
    QueueThreads = new std::queue<int>();
    interval = timeslice;
    _ticked = 0;
    _current_thread_id = -1;
}
...
void timer_tick()
{
    ...
    if (++_ticked == interval)
    {
        QueueThreads->push(_current_thread_id);
        switch_to_next_thread();
    }
}
